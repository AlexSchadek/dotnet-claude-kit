...
- `self-correction-loop` — Capture corrections as permanent rules in .github/MEMORY.md
- `wrap-up-ritual` — Structured session handoff to `handoff.md`
- `context-discipline` — Token budget management, MCP-first navigation

## MCP Tools

> **Setup:** Build the Roslyn MCP server (`dotnet build mcp/CWM.RoslynNavigator`) and create `.vscode/mcp.json` in your repo with:
> ```json
> {
>   "servers": {
...
>       "type": "stdio",
>       "command": "dotnet",
>       "args": ["run", "--project", "<path-to>/mcp/CWM.RoslynNavigator", "--", "--solution", "${workspaceFolder}"]
>     }
>   }
> }
> ```
> VS Code auto-loads this on workspace open. After that, the Roslyn tools are available in Copilot Chat.
